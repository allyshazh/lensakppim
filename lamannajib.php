...
if (isset($_POST['add_comment'])) {
    ...
    $stmt = $connect->prepare("INSERT INTO lamannajib (userid, comment, date) VALUES (?, ?, NOW())");
...
if(isset($_POST['update_now'])){
   ...
   $verify_comment = $connect->prepare("SELECT * FROM `lamannajib` WHERE commentid = ? AND comment = ?");
   $verify_comment->execute([$update_id, $update_box]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'Comment Already Added!';
   }else{
      $update_comment = $connect->prepare("UPDATE `lamannajib` SET comment = ? WHERE commentid = ?");
...
if (isset($_POST['delete_comment'])) {
    ...
    $stmt = $connect->prepare("DELETE FROM lamannajib WHERE commentid = ? AND userid = ?");
...
if(isset($_POST['edit_comment'])){
   ...
   $verify_comment = $connect->prepare("SELECT * FROM `lamannajib` WHERE commentid = ? LIMIT 1");
...
<section class="edit-comment">
   ...
   <form action="" method="post">
      ...
      <div class="flex">
         <a href="lamannajib.php?landmark_id=<?= $landmark_id ?>" class="inline-option-btn">Cancel Edit</a> <!-- Update link with landmark_id -->
...
<section class="landmark-details">
   <h1 class="heading">Laman Najib</h1>
   ...
   <div class="row">
      <div class="column">
         ...
         <div class="thumb">
            <img src="images/laman-najib-2.jpg" alt="">
      ...
      <div class="column">
         <div class="details">
            <h3>Faculty Lounge</h3>
            <p>A social hub where students, lecturers, and staffs of KPPIM faculty can hang out together. 
               It works as a central point for any activities or events organised by KPPIM.
               At Laman Najib, one can find small stalls that sell snacks and lunch packs.
               Individuals can engage in informal discussions, study group and even leisure activities there.</p>
         </div>
      </div>
   </div>
</section>
...
<section class="comments">
    ...
    <div class="box-container">
       ...
       $select_comments = $connect->prepare("SELECT lamannajib.*, students.studentname, students.studentimage 
                                             FROM `lamannajib` 
                                             JOIN `students` 
                                             ON lamannajib.userid = students.userid 
                                             ORDER BY lamannajib.date DESC"); // Assuming you want to order comments by date
