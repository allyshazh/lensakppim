...
if (isset($_POST['add_comment'])) {
    ...
    $stmt = $connect->prepare("INSERT INTO alkhawarizmi (userid, comment, date) VALUES (?, ?, NOW())");
...
if(isset($_POST['update_now'])){
   ...
   $verify_comment = $connect->prepare("SELECT * FROM `alkhawarizmi` WHERE commentid = ? AND comment = ?");
   $verify_comment->execute([$update_id, $update_box]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'Comment Already Added!';
   }else{
      $update_comment = $connect->prepare("UPDATE `alkhawarizmi` SET comment = ? WHERE commentid = ?");
...
if (isset($_POST['delete_comment'])) {
    ...
    $stmt = $connect->prepare("DELETE FROM alkhawarizmi WHERE commentid = ? AND userid = ?");
...
<head>
   ...
   <title>Al-Khawarizmi Building</title>
...
if(isset($_POST['edit_comment'])){
   ...
   $verify_comment = $connect->prepare("SELECT * FROM `alkhawarizmi` WHERE commentid = ? LIMIT 1");
...
<section class="edit-comment">
   ...
   <form action="" method="post">
      ...
      <div class="flex">
         <a href="alkhawarizmi.php?landmark_id=<?= $landmark_id ?>" class="inline-option-btn">Cancel Edit</a> <!-- Update link with landmark_id -->
...
<section class="landmark-details">
   <h1 class="heading">Al-Khawarizmi Building</h1>
   ...
   <div class="row">
      <div class="column">
         ...
         <div class="thumb">
            <img src="images/al-khawarizmi-2.jpg" alt="">
      ...
      <div class="column">
         <div class="details">
            <h3>Known as The CS2 Building</h3>
            <p>Formerly was the Dentistry Faculty until they were transfered to the UiTM Sungai Buloh campus.
            The building serves as a central hub for hosting classrooms, laboratories, and offices where teaching, learning, and research activities occur.
            It is a place for faculty and students to engage in academic discussions and social interactions, 
...
<section class="comments">
    ...
    <div class="box-container">
       ...
       $select_comments = $connect->prepare("SELECT alkhawarizmi.*, students.studentname, students.studentimage 
                                             FROM `alkhawarizmi` 
                                             JOIN `students` 
                                             ON alkhawarizmi.userid = students.userid 
                                             ORDER BY alkhawarizmi.date DESC"); // Assuming you want to order comments by date
