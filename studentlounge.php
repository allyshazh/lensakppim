...
if (isset($_POST['add_comment'])) {
    ...
    $stmt = $connect->prepare("INSERT INTO studentlounge (userid, comment, date) VALUES (?, ?, NOW())");
...
if(isset($_POST['update_now'])){
   ...
   $verify_comment = $connect->prepare("SELECT * FROM `studentlounge` WHERE commentid = ? AND comment = ?");
   $verify_comment->execute([$update_id, $update_box]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'Comment Already Added!';
   }else{
      $update_comment = $connect->prepare("UPDATE `studentlounge` SET comment = ? WHERE commentid = ?");
...
if (isset($_POST['delete_comment'])) {
    ...
    $stmt = $connect->prepare("DELETE FROM studentlounge WHERE commentid = ? AND userid = ?");
...
if(isset($_POST['edit_comment'])){
   ...
   $verify_comment = $connect->prepare("SELECT * FROM `studentlounge` WHERE commentid = ? LIMIT 1");
...
<section class="edit-comment">
   ...
   <form action="" method="post">
      ...
      <div class="flex">
         <a href="studentlounge.php?landmark_id=<?= $landmark_id ?>" class="inline-option-btn">Cancel Edit</a> <!-- Update link with landmark_id -->
...
<section class="landmark-details">
   <h1 class="heading">Student Lounge</h1>
   ...
   <div class="row">
      <div class="column">
         ...
         <div class="thumb">
            <img src="images/student-lounge-2.jpg" alt="">
      ...
      <div class="column">
         <div class="details">
            <h3>Student Hub</h3>
            <p>A dedicated space designed to provide KPPIM students with a place to relax, socialize, and study
               outside of their formal classroom environments. It is equipped with comfortable seating, tables, plugpoints
               and often amenities like vending machines, water dispenser. It functions as a venue for various student activities 
               and events to enrich the campus life.</p>
         </div>
      </div>
   </div>
</section>
...
<section class="comments">
    ...
    <div class="box-container">
       ...
       $select_comments = $connect->prepare("SELECT studentlounge.*, students.studentname, students.studentimage 
                                             FROM `studentlounge` 
                                             JOIN `students` 
                                             ON studentlounge.userid = students.userid 
                                             ORDER BY studentlounge.date DESC"); // Assuming you want to order comments by date
